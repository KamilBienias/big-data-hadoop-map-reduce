# Etap9. Odcinek: JupyterNotebook: Eksploracja wynikow (Plotly) drugi
...
df = pd.read_csv('/home/dell/PycharmProjects/big-data-hadoop-map-reduce/04_airports/average_delay_by_airline.csv',
                 sep='\t',
                 names=['airline', 'to_split'])
...
fig = px.bar(df, x='airline', y='dep_delay', orientation='v', color='airline')
fig.show()

fig = px.bar(df, x='airline', y='arr_delay', orientation='v', color='airline')
fig.show()
...
fig = go.Figure(data=[
                     # pierwszy slad
                     go.Bar(x=df.airline, y=df.dep_delay, name='departure delay', marker_color='#517CAA'),
                     # drugi slad
